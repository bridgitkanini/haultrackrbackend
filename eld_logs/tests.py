...
class LogSheetModelTests(TestCase):
    ...
    def test_total_on_duty_hours(self):
        ...
        self.assertEqual(self.log_sheet.total_on_duty_hours, 4.25)  # 4 hours driving + 15 min on-duty
...
class LogGeneratorTests(TestCase):
    ...
    def test_trip_segmentation(self):
        ...
        self.assertEqual(len(segments), 4)  # Should have 4 segments (drive, rest, drive, fuel)
...
class LogSheetAPITests(APITestCase):
    def setUp(self):
        # Create test data similar to LogSheetModelTests
        self.trip = Trip.objects.create(
            current_location="New York, NY",
            pickup_location="Chicago, IL",
            dropoff_location="Los Angeles, CA",
